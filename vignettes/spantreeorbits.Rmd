...
In Figure 1a we see that the *get_orbits_of_trees* function has separated the spanning trees into kinds: 1a has all of the *star3* trees. There are 4 such spanning trees, corresponding to the 4 vertices of the tetrahedron.

```{r display1b, fig.align='center', fig.cap=c("Figure 1b. A tetrahedron and its dual, with some spanning trees, as red edges.")}
display_tree_pairs(tetrahedron, dual_tetrahedron, rhombic_dodecahedron, reps, orbits[[1]][c(1,12,2,11,3,10)])

```

```{r display1c, fig.align='center', fig.cap=c("Figure 1c. A tetrahedron and its dual, with some spanning trees, as red edges.")}
display_tree_pairs(tetrahedron, dual_tetrahedron, rhombic_dodecahedron, reps, orbits[[1]][c(4,9,5,8,6,7)])
```

Figures 1b and 1c contain all of the spanning trees which are *line3* trees.

The left and right parts of Figures 1b and 1c are the same, except that the tetrahedra and the duals are switched. There are 6 such pairs of spanning trees, corresponding to the $ _4C_2 = 6 $ ways that the 2 ends of a *line3* tree can be mapped to the 4 vertices of the tetrahedron.

# Another Example

We consider the cube,  with 8 vertices, 6 faces, and 12 edges, and its dual polyhedron, the octahedron.
